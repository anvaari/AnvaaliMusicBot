...
async def delete_track_handler(callback: CallbackQuery,state: FSMContext):
    ...
    if not tracks:
        ...
        return await callback.answer("Playlist is empty", show_alert=True)
    ...
    await callback_message.answer(
        ...
        reply_markup=music_remove_keyboard
    )
...
async def delete_track(callback: CallbackQuery, state: FSMContext):
    ...
    if success == True:
        ...
        await callback_message.answer(f"✅ Track #{track_index} removed from '{playlist_name}'.")
    else:
        logger.warning(f"Failed to remove track #{track_index} from '{playlist_name}' for user_id={user_id} ")
        await callback_message.answer(f"❌ Can't remove #{track_index} from *{playlist_name}*")
